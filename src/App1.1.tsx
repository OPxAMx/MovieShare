  ...
  if (authLoading) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode 
        ...
        <div className={`text-xl ${darkMode ? 'text-white' : 'text-slate-900'}`}>Chargement...</div>
      </div>
    );
  }
        ...
        <div className={`container mx-auto px-4 ${headerCollapsed ? 'py-2' : 'py-4'} transition-all duration-300`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">MovieShare</h1>
          ...
          {!headerCollapsed && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                <div className={`rounded-lg p-3 ${darkMode ? 'bg-slate-700/50' : 'bg-white/70'}`}>
                  <div className={`flex items-center gap-2 text-sm mb-1 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                    <Film size={16} />
                    Total
                  </div>
              ...
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1 relative">
                  <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`} size={20} />
                  <input
                    type="text"
                    value={searchQuery}
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {moviesLoading ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-xl">Chargement des films...</div>
          </div>
        ) : filteredAndSortedMovies.length === 0 ? (
          <div className={`flex flex-col items-center justify-center py-20 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {movie.year && (
                        <span className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>({movie.year})</span>
                      )}
                      {movie.is_favorite && (
                    ...
                    <div className="flex items-center gap-4 mb-3">
                      ...
                      {movie.duration && (
                        <span className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{movie.duration}</span>
                      )}
                    ...
                    {movie.description && (
                      <p className={`text-sm line-clamp-2 mb-3 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                        {movie.description}
                      </p>
                    )}
